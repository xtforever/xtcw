...
int FreeXfwfPen(Widget w, XfwfPen * pen)
{
	...
	if (pen->colors) {
		XFreeColors(dpy, w->core.colormap, pen->colors, pen->ncolors,
			    0L);
...
static int
convert_pixel(Display * dpy, Screen * screen, Colormap cmap, char *name,
	      unsigned long *pixel)
{
...
Boolean ConvertStringToPen(Display * dpy, Screen * screen, Colormap colormap, Widget w,	/* could be an Object */
			   char *src, XfwfPen * dest)
{
	char *scan, *s;
	...
	while (1) {
		...
		*scan = 0;
		...
		if (0 == XmuCompareISOLatin1(s, "solid")) {
			...
			if (!convert_pixel
			    (dpy, screen, colormap, scan,
			     &dest->xgcv.foreground))
				break;
			...
			*scan = 0;
			...
			if (!convert_pixel
			    (dpy, screen, colormap, s, &dest->xgcv.foreground))
				break;
			dest->mask |= GCForeground;

			dest->xgcv.stipple =
			    XmuLocateBitmapFile(screen, scan, (char *)0, 0,
			...
			*scan = 0;
			...
			if (!convert_pixel
			    (dpy, screen, colormap, s, &dest->xgcv.background))
				break;
			...
			if (0 == XmuCompareISOLatin1(scan, "XtCheckerBitmap")) {
				...
				dest->xgcv.stipple =
				    XmuLocateBitmapFile(screen, scan, (char *)0,
							0, &dummy, &dummy,
							&dummy, &dummy);
			}
...
#if 0
			{
				XrmValue to, from;
				from.addr = scan;
				from.size = ? to.addr = &dest->xgcv.tile;
				to.size = sizeof(dest->xgcv.tile);
				XtConvertAndStore(w, XtRString, &from,
						  XtRPixmap, &to);
			}
#else
			if (!XfwfLocatePixmapFile
			    (screen, colormap, visual, scan,, (Pixmap *) 0,
			     (char **)0, (int *)0, (int *)0, &dest->colors,
			     &dest->ncolors))
				break;
#endif
			dest->mask |= GCTile;
			...
			*scan = 0;
			...
			depth = ((XtIsWidget(w)) ? w : XtParent(w))->core.depth;
...
Boolean
XfwfCvtStringToPen(Display * dpy,
		   XrmValue * args,
		   Cardinal * num_args,
		   XrmValue * fromVal,
		   XrmValue * toVal, XtPointer * converter_data)
{
	Screen *screen;
	Colormap colormap;
	Object o;
...
/*  char		*src;*/
	...
	dest = *(XfwfPen **) toVal->addr =
	    (XfwfPen *) XtMalloc(sizeof(XfwfPen));
...
void
XfwfStringToPenDestructor(XtAppContext app, XrmValue * to,
