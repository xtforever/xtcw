...
#define CHECK(a) show_error( (a), #a )
void show_error( int err, char *expr )
{
    if( err < 0 ) {
...
int wav_read(char *fn, char **buf1, int *len)
{
    struct stat sb;
    int fd;
    ...
    assert( close(fd) == 0);
    *buf1 = buf + sizeof(WavHeader); *len = sb.st_size - sizeof(WavHeader);
    return 0;
}

struct wav_data {
    char *buf;
    unsigned pos, len, period_size;
};

int snd_update_cache(snd_pcm_t *handle,struct wav_data *data)
{
    int err;
    snd_pcm_sframes_t avail, nframes;

    if( data->pos >= data->len ) return -1;
    avail = snd_pcm_avail_update(handle);
    printf("%s: avail %u\n", __func__, (unsigned)avail );
    if( avail > 0 ) {
        if( avail > (data->len - data->pos)/4 )
            avail = (data->len - data->pos)/4;
...
static void wav_async_callback(snd_async_handler_t *ahandler)
{
        snd_pcm_t *handle = snd_async_handler_get_pcm(ahandler);
        struct wav_data *data = snd_async_handler_get_callback_private(ahandler);
        if( snd_update_cache(handle, data) ) /* ende */
            {
...
void simple_init(void)
{
    ...
    assert( wav_read("test.wav", &buf,&len) == 0 );
    data.buf = buf; data.pos = 0; data.len = len; data.period_size = 4096;
    snd_pcm_hw_params_alloca(&hwparams);
    snd_pcm_sw_params_alloca(&swparams);
    CHECK(snd_output_stdio_attach(&output, stdout, 0));
    CHECK(snd_pcm_open(&handle, device, SND_PCM_STREAM_PLAYBACK, 0));
    CHECK(set_hwparams(handle, hwparams,SND_PCM_ACCESS_RW_INTERLEAVED));
    CHECK(set_swparams(handle, swparams));
    CHECK(snd_async_add_pcm_handler(&ahandler, handle, wav_async_callback, &data));
    CHECK(snd_pcm_nonblock(handle, 1));
    snd_update_cache(handle,&data);
