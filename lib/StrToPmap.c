...
#define PIXMAPDIR "/usr/share/pixmaps"
  ...
  for (i = 1; i <= 4; i++) {
    ...
    case 1:
      ...
      */
      ...
      if(!(  
         (name[0]=='/') /* case 1 */
         || 
         (name[0]=='.' && name[1]=='/' ) /* case 2 */
         ||
         (name[0]=='.' && name[1]=='.' && name[2]=='/' ) /* case 3 */
             )) continue;
      fn = name;
	...
	snprintf (filename, sizeof(filename), "%s/%s", *file_paths, name);
	...
	break;
      ...
      continue;
    case 3:
      snprintf (filename, sizeof(filename), "%s/%s", PIXMAPDIR, name);
      break;
    case 4:
